...
this entry is the executable half. Authoring rules for the handouts themselves live in
[`handout-authoring.md`](./handout-authoring.md) — the instructor writes them, the app never
...
Each student receives their own version of a handout — lecture-notes prose, LLM-generated by the
instructor outside the app. After every section the student judges it. Some sections are then
shown a second version and asked which they would rather have learned from. The instructor reads
the result and writes better handouts; over a term the records accumulate into a preference
dataset in a shape that could later support reward modelling or DPO.

The design document is right that this is **an evaluation system first and a training corpus
second**, and the numbers say so plainly: six students × ~5 sections × ~10 handouts is a few
hundred records. That is far too few to train anything from scratch and genuinely useful as a
held-out set for answering "did changing the generation prompt produce material students
prefer?". Build it for the second purpose; keep the schema shaped for the first.
...
`handout.json`. With six students and two versions this is an exact Latin square — every student
sees A/B/A/B or B/A/B/A, every section is seen in both versions by three students each. Balanced
and counterbalanced by construction, which random assignment only approaches in expectation and
...
ratings silently describe text that no longer exists. `sha256(body).slice(0, 16)`, stamped at
...
arrive the way readings do — uploaded through the app by the instructor. Eight to twelve files
per handout makes file-at-a-time upload tedious, so `npm run bundle:handout <dir>` validates a
...
**6. The probe runs at the end, not inline.** Interrupting a student mid-handout to read a second
version of section 2 breaks the reading. At the foot of the page: "here is another way we could
have written §2 — which would you rather have learned from?", three answers (the one they read,
the alternate, no preference). The alternate is `(assigned + 1) % versions.length`, so it is
deterministic too. The record carries both version ids **and both content hashes**, because a
preference is a claim about two specific pieces of text.

### The commits

| # | Commit | What changes | Files |
|---|---|---|---|
| 4a | **The handout store and its bundler** | Bundle format, loader, and `npm run bundle:handout` — which is also the validator: it refuses mismatched titles across versions, a `probes` entry naming no section, a missing `generation` block, or a duplicate `section_id`. Admin-only upload, mirroring 2e's guard exactly. | `virtual_space/server/handouts.ts`, `scripts/bundle-handout.ts`, `server/index.ts`, `package.json` |
...
| 4c | **The section widget and the write path** | 👍/😐/👎 after each section; 😐 or 👎 reveals the tag list and a one-line comment. Autosaves on click, no submit button, restores on reload. `POST /api/handouts/:id/feedback` takes the student from `identify()` and **never** from the body. | `virtual_space/server/handouts.ts`, `server/index.ts`, `server/render.ts` |
| 4d | **The pairwise probe** | The end-of-handout comparison, and the preference record. | `virtual_space/server/handouts.ts`, `server/render.ts` |
| 4e | **The instructor's view** | Admin-only `GET /admin/handouts/:id`: response rate per section, thumbs split, tag counts, comments verbatim, probe tallies — ordered worst-first, aggregated by section rather than by person. `?format=jsonl` exports the training-shaped records. | `virtual_space/server/handouts.ts`, `server/index.ts`, `client/src/main.ts` |
...
```json
{
  ...
  "comment": "…",
  "preference": { "chosen": "B", "rejected": "A", "chosen_sha": "…", "rejected_sha": "…" },
  "ts": "2026-09-02T14:11:03Z"
}
```

`generation` is copied onto the record rather than referenced, so an exported line is
self-contained and a later edit to the source file cannot rewrite the past.

### Consequences to accept

1. **Handouts stay out of the TA's corpus.** If the TA can read the variants it will describe the
   other version to a student and contaminate the comparison. The cost is that the TA cannot help
   with the handout — accepted deliberately. A `promote` action, moving a winning version into the
   Library corpus once the comparison is done, is the right closing move and is **not** in this
   plan.
...
4. **Six students is the whole sample.** One student not responding is 17% of the data. The
   instructor has said response will be required, which is what makes the design viable at all.
...
- all six students, all sections: **each version appears an equal number of times**, and each
  student sees each version equally often across sections;
- a student reloading gets **the same** versions — the assignment is read back, not re-derived;
- adding a seventh section, or a third version, does not change what an existing student already
  saw;
...
dwell time; anything resembling model training. And the deploy, which this depends on rather
than includes.
