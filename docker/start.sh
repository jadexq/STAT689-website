#!/bin/bash
# A container has one PID 1 but we need two servers. Start the TA brain,
# wait for it to answer, then start the space — and make sure the container
# DIES if either one does, so Cloud Run replaces it instead of serving a
# half-working campus.
#
...
# Exit as soon as EITHER process exits, carrying its status out with us.
set +e
wait -n "$TA_PID" "$SPACE_PID"
STATUS=$?
echo "[start] a server exited (status $STATUS) — shutting the container down" >&2
kill "$TA_PID" "$SPACE_PID" 2>/dev/null
exit "$STATUS"
